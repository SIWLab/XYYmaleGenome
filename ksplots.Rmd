...
```{r}
library(slider)
 windowmaker<- function(df,win){ks_df_md %>% filter(Ks<0.2) %>%
  group_by(chr1) %>%
...
ksplot<-ggplot(filter(ks_win100,!grepl("scaffold", chr1)), aes(start1_mb,group = chr1)) +  
...
##
max_query_mb <- function(chr){max(filter(.,newChr ==chr)$queryMb)}
max_query_mb("Y")                          
...
dotplot<-ggplot(anchors_inv,aes(referenceMb,newstartMb)) + 
  ...
  geom_text(data = dat_text,size=2,mapping = aes(x = x, y = y, label = label)) +
  ylim(0,430) +
...
To do:
* fix axis points
* why is start position not zero for A2-X??? need to adjust how I've flipped the chr...
* grey label only, top, remove middle
* wa hoo!
------
