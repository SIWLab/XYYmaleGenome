...
pairedTest <- pairedGenes %>%
  ...
  group_by(location, scaffH1, scaffH2, winnum) %>%
  summarise(freqH1 = mean(frequencyH1), 
            freqH2 = mean(frequencyH2), .groups = "keep") %>% 
  drop_na(freqH1)

pairedTest %>%
  filter(location == "downstream") %>%
  ggplot() +
  geom_point(aes(x = winnum, y = freqH1), colour = pubColours[1], alpha = 0.5) +
  geom_point(aes(x = winnum, y = freqH2), colour = pubColours[2], alpha = 0.3) +
  scale_colour_manual(values = pubColours) +
  pubTheme + theme(legend.position = c(0.85, 0.5)) +
  labs(x = "Position (1Mb windows)", y = "TE frequency") +
