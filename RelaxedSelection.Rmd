...
pairedFreq <- pairedGenes %>%
  group_by(scaffH1, scaffH2, location) %>%
  select(location:scaffH2, densityH1, densityH2) %>%
  summarise(meanH1 = mean(densityH1), meanH2 = mean(densityH2), n = n(),
            medianH1 = median(densityH1), medianH2 = median(densityH2),
            .groups = "keep") %>%
  drop_na() # still very concerned about this
...
pairedGenes %>%
  ...
  select(location:scaffH2, densityH1, densityH2) %>%
  drop_na() %>%
  pivot_longer(cols = c(densityH1, densityH2), names_to = "Haplotype", 
               values_to = "frequency") %>%
