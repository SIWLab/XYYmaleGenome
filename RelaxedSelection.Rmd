...
Merge the H1 and H2 data by 1:1 ortholog pairs
...
genewise <- function(H1list, H2list, pairList) {
  {
    H1tibble <- H1list %>%
      ...
      # There are duplicates because of the ortholog data formatting
      distinct() %>%
      # Rename the location var so the values are meaningful & extract the
      #gene name from the attribute column
      ...
      inner_join(pairList, by = join_by(H1))
  } # Make the H1 tibble
  {
    H2tibble <- H2list %>%
      list_rbind(, names_to = "location") %>%
      ...
      inner_join(pairList, by = join_by(H2))
  } # Make the H2 tibble
...
Region "sizes" = sum of all element bps in the region
...
allRegionSizes <- full_join(XregionTEs, Y1regionTEs, 
                          by = join_by(region, location)) %>%
  full_join(Y2regionTEs, by = join_by(region, location)) %>%
  pivot_longer(cols = c(X, Y1, Y2), 
...
pairedGenesMulti <- genewise(H1list, H2list, pairList)

# Summarise the exons/introns per gene
pairedGenes <- pairedGenesMulti %>% 
  group_by(H1, H2, location, scaffH1, scaffH2) %>%
  summarize(startH1 = robustMin(startH1), countH1 = sum(countH1), 
            TElenH1 = sum(TElenH1), eleLenH1 = sum(eleLenH1),
            frequencyH1 = mean(densityH1), 
            startH2 = robustMin(startH2), countH2 = sum(countH2), 
            TElenH2 = sum(TElenH2), eleLenH2 = sum(eleLenH2),
            frequencyH2 = mean(densityH2), 
            .groups = "keep")
```
...
pairedGenes %>%
  ...
  filter(scaffH1 == "X") %>%
  ggplot() +
  geom_point(aes(x = frequencyH1, y = frequencyH2, colour = location), 
             alpha=0.3) +
  geom_abline() +
  pubTheme + scale_colour_manual(values = pubColours) + 
  theme(legend.position = "none") +
  labs(x = "TE frequency for H1", y = "TE frequency for H2", colour = "Size") +
...
```{r summingBPforBarplots, include=FALSE, eval=FALSE}
#%>%
