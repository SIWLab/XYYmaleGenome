...
```{r}
a1<- anchors_mut %>% filter(newChr=="A1") %>%  ggplot(aes(referenceStart,newstart)) + geom_point() + theme_classic()
## need to transform A2, A4 and Y for some reason...
a2<-anchors_mut %>% filter(newChr=="A2") %>% mutate(newstart=(max(queryStart) - queryStart)) %>% ggplot(aes(referenceStart,newstart)) + geom_point() + theme_classic()
a4<-anchors_mut %>% filter(newChr=="A4") %>% mutate(newstart=(max(queryStart) - queryStart)) %>%ggplot(aes(referenceStart,newstart)) + geom_point() + theme_classic()
y<-anchors_mut %>% filter(newChr=="Y") %>%  mutate(newstart=(max(queryStart) - queryStart)) %>% ggplot(aes(referenceStart,newstart)) + geom_point(aes(color=queryChr)) + theme_classic() 
 
plot_grid(a1,a2,a4,y)
```
