...
y<-anchors_mb %>% filter(newChr=="Y") %>%
  ...
  annotate("text", x = .35*(71), y = 400, label = "PAR1") +
  annotate("text", x = .5*(71+261), y = 400, label = "Old sex-linked\n region") +
  annotate("text", x = .5*(261+367), y = 400, label = "New sex-linked\n region") +
  annotate("text", x = .5*(367+500), y = 400, label = "PAR2") +
  ylim(0,430) +
  theme_classic() +
  theme(legend.position = c(0.9,0.3)) +
  scale_color_manual(name="Y chromosome",
                    labels=c("old Y","neo Y"),
...
y.grob <- textGrob("Maternal Haplotype",gp=gpar(fontface="bold", fontsize=14), rot=90)

x.grob <- textGrob("Paternal Haplotype",gp=gpar(fontface="bold", fontsize=14))

#add to plot

grid.arrange(arrangeGrob(myplot, left = y.grob, bottom = x.grob))
