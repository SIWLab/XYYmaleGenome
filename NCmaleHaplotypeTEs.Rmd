...
# 10 colours, to cover all options for TE types
opt2 <- c('#2c31c2', '#405cc8', '#5182ce', '#66a7d4', '#82cbda',
          '#edd271', '#d8a650', '#c2793c', '#ab4b36', '#93003a')
opt3 <- c('#00429d', '#3e67ae', '#618fbf', '#85b7ce', '#b1dfdb', 
          '#bed272', '#a0ae55', '#928642', '#90593a', '#93003a')
#best colour divergence
opt1 <- c("#4200de", "#1a63ca", "#0792c1", "#2bbacd", "#72dde6",
          "#c7dd85", "#90bb6a", "#679752", "#75674b", "#93003a")
pubColours <- c("#90bb6a", "#1a63ca", "#0792c1", "#75674b", "#c7dd85",
                "#4200de", "#679752", "#93003a", "#2bbacd", "#72dde6")

#pubColours <- c("#65bc5c","#01a5bc","#516fc8","#d84638") #sex chromosome-focussed
...
And the points at which to divide the sex chromosomes by their sex linkage strata (just for making dotted lines on facetted plots)

```{r sexChromDivs}
Xdiv <- expand.grid(scaff = factor(c("X")), pos = c(61, 261, 367))
Y1div <- expand.grid(scaff = factor(c("Y1")), pos = c(64, 272))
Y2div <- expand.grid(scaff = factor(c("Y2")), pos = c(121, 159))
```
...
```{r filterLiftedGenesFunction}
checkLifted <- function(original, lifted, oscaffs, lscaffs) {
  keepers <- lifted %>% 
    filter(scaff %in% lscaffs, seqOnt == "gene") %>%
    mutate(ID = str_match(attr, "ID=([^;]+)")[,2]) %>%
    pull(ID)
  
  original %>%
    filter(scaff %in% oscaffs) %>%
    #select(!c(seqOnt, score, strand, phase)) %>%
    filterGenes %>%
    filter(str_detect(attr, str_c(keepers, collapse = "|")))
}
```
...
summarizeGenes <- function(df) {
  df %>%
    ...
    summarise( bp = sum(length), n = n(), .groups = "keep" ) %>%
...
### List of TE families with at least # members
...
```{r filterGenesForLiftover, include=FALSE}
H1GenesLiftedWell <- 
  checkLifted(H1GenesRaw, H2LiftedRaw, H1scaffs, H2scaffs) %>%
  window %>%
  summarizeGenes
  
H2GenesLiftedWell <- 
  checkLifted(H2GenesRaw, H1LiftedRaw, H2scaffs, H1scaffs) %>%
  window %>%
  summarizeGenes


checkLifted(H1GenesRaw, H2LiftedRaw, H1scaffs, H2scaffs) %>%
  write_tsv("data/H1GenesLiftedWell.tsv")
checkLifted(H2GenesRaw, H1LiftedRaw, H2scaffs, H1scaffs) %>%
  write_tsv("data/H2GenesLiftedWell.tsv")
```
...
```{r genesInSexRegions}
XsummedGenes <- H1GenesLiftedWell %>%
  filter(scaff == "X") %>%
  splitX %>%
  group_by(region) %>%
  summarize(summedbp = sum(bp), .groups = "keep") %>%
...
### TE Frequency and Count per scaffold
...
### TE Sex chromosome proportions

The first plot (with region on the x-axis) feels like the best way to visualise the comparisons we're interested in. 

```{r actualSummedTEsPlot}
TEsexChromosomes <- allSummedTEs %>%
  mutate(region = case_when(region == "PAR1" ~ "PAR",
                            region == "PAR2" ~  "PAR",
                            .default = region)) %>%
  ggplot(aes(x = region, y = bp, fill = Chromosome)) +
  geom_col(position = position_dodge()) +
...
### Gene Sex chromosome proportions
...
### Separate plots for H1 and H2 of both TE frequenc and count

```{r plotSeparatelyPerHaplotype}
...
landscapeFraction <- H1EDTA %>%
  filter(scaff == "X") %>%
  ggplot() +
  # Creating dividing lines for sex linkage on sex chromosomes, with text labels to indicate whether old sex-linked, new sex-linked, or pseudoautosomal
  geom_vline(data = Xdiv, aes(xintercept = pos), linetype = "dotted",
             colour = "#152944") +
  geom_vline(data = Y1div, aes(xintercept = pos), linetype = "dotted",
             colour = "#152944") +
  geom_vline(data = Y2div, aes(xintercept = pos), linetype = "dotted",
             colour = "#152944") +
  geom_text(data = Xlabels, aes(x = pos, y = 0.9, label = stratum), 
            colour = "#152944") +
  geom_text(data = Y1labels, aes(x = pos, y = 0.9, label = stratum), 
            colour = "#152944") +
  geom_text(data = Y2labels, aes(x = pos, y = 0.9, label = stratum), 
            colour = "#152944") +
  # Plot the autosomal and Y TE frequencies (H2)
  geom_line(data = H2EDTA, aes(winnum, amt, colour = seqOnt), 
            linewidth = 0.3) +
  # Plot the X TE frequencies (H1)
  geom_line(aes(winnum, amt, colour = seqOnt), linewidth = 0.3) +
  # Plot the H2 gene frequencies
  geom_line(data = H2Genes, aes(winnum, amt)) +
  # Plot the X gene frequencies using the H1 genes that lifted well to H2
  geom_line(data = filter(H1GenesLiftedWell, scaff == "X"),
            aes(winnum, amt)) + 
  # Formatting
  pubTheme + #scale_colour_manual(values = pubColours) +
  theme(legend.position = "none") + #covered by the last panel
...
H1EDTA %>%
  ...
  geom_text(data = Y2labels, aes(x = pos, y = 0.9, label = stratum), 
            colour = "#152944") +
  # Plot the autosomal and Y TE frequencies (H2)
  geom_line(data = H2EDTA, aes(winnum, log(amt), colour = seqOnt), 
            linewidth = 0.3) +
  # Plot the X TE frequencies (H1)
  geom_line(aes(winnum, log(amt), colour = seqOnt), linewidth = 0.3) +
  # Plot the H2 gene frequencies
  geom_line(data = H2Genes, aes(winnum, log(amt))) +
  # Plot the X gene frequencies using the H1 genes that lifted well to H2
  geom_line(data = filter(H1GenesLiftedWell, scaff == "X"),
            aes(winnum, log(amt))) + 
  # Formatting
  pubTheme + scale_colour_manual(values = pubColours) +
  labs(x = "Position (1Mb windows)", y = "Log Fraction of window", 
       colour="TE Type") +
  facet_grid(vars(scaff), scale="free_x", space="free")
```
...
H1EDTA %>%
  ...
  geom_vline(data = Y2div, aes(xintercept = pos), linetype = "dotted",
             colour = "#152944") +
  geom_text(data = Xlabels, aes(x = pos, y = 6000, label = stratum), 
            colour = "#152944") +
  geom_text(data = Y1labels, aes(x = pos, y = 6000, label = stratum), 
            colour = "#152944") +
  geom_text(data = Y2labels, aes(x = pos, y = 6000, label = stratum), 
            colour = "#152944") +
  geom_line(data = H2EDTA, aes(winnum, n, colour = seqOnt), 
            linewidth = 0.3) +
  geom_line(aes(winnum, amt, colour = seqOnt), linewidth = 0.3) +
  pubTheme +
  labs(x = "Position (1Mb windows)", y = "Number of TEs", 
       colour="TE Type") +
  facet_grid(vars(scaff), scale="free_x", space="free")
```
...
H1EDTA %>%
  ...
  geom_vline(data = Y2div, aes(xintercept = pos), linetype = "dotted",
             colour = "#152944") +
  geom_text(data = Xlabels, aes(x = pos, y = 8, label = stratum), 
            colour = "#152944") +
  geom_text(data = Y1labels, aes(x = pos, y = 8, label = stratum), 
            colour = "#152944") +
  geom_text(data = Y2labels, aes(x = pos, y = 8, label = stratum), 
            colour = "#152944") +
  geom_line(data = H2EDTA, aes(winnum, log(n), colour = seqOnt), 
            linewidth = 0.3) +
  geom_line(aes(winnum, log(n), colour = seqOnt), linewidth = 0.3) +
  pubTheme +
  labs(x = "Position (1Mb windows)", y = "Log number of TEs", 
       colour="TE Type") +
  facet_grid(vars(scaff), scale="free_x", space="free")
```
...
H1TEs %>%
  ...
  geom_text(aes(label = n + 2), size = 3,  angle = 85) +
...
### Sex chromosome Family size differences

Bar plots of the largest families

```{r barplotBiggestFams}
# # Using haplotype as a factor (which is probably cleaner?)
biggestFamBars <- famFacTally %>%
  group_by(family) %>%
  arrange(desc(total)) %>%
  head(80) %>%
  ggplot(aes(x = reorder(family, -total), y = total / 1000000)) +
  geom_col(aes(fill = seqOnt, y = fraction, colour = haplotype), 
           linewidth = 0.9, width = 0.5) +
  scale_colour_manual(values = c("light grey", "black")) +
  #geom_text(aes(y = fraction + 0.02, label = count), size = 3, angle = 85) +
  pubTheme + theme(legend.position = c(0.94, 0.59),
                   axis.text.x = element_blank(), 
                   axis.ticks.x = element_blank()) +
  labs(x = "40 largest TE families on the sex chromosomes",
       y = "Fraction of the sex chromosome occupied", 
       colour = "Chromosome ", fill = "TE Type")
biggestFamBars

# Using points to differentiate haplotypes
shapes <- c("H1" = 16, "H2" = 17)
famTally %>%
  ...
  geom_col(aes(fill = seqOnt), width = 0.8) +
  # Label columns with copy number across haplotypes
  geom_text(aes(y = bp / 1000000 + 2.8, label = n), size = 3,  angle = 85) +
  # Use points to differentiate haplotypes: 16 is a circle, 17 a triangle
  geom_point(aes(y = bpH1 / 1000000, shape = "H1"), size = 2.5, stroke = 1.2) +
  geom_point(aes(y = bpH2 / 1000000, shape = "H2"), size = 2.5, stroke = 1.2) +
  pubTheme + theme(legend.position = c(0.95, 0.62),
                   axis.text.x = element_blank(),
                   axis.ticks.x = element_blank()) +
  labs(x = "40 largest TE families on the sex chromosomes",
       y = "Size (Mb occupied)", fill = "TE Type", shape = "Haplotype") +
  scale_shape_manual(values = shapes)
```

### Scatterplot of family size on Haplotype 1 and Haplotype 2

This now accounts for the total size of the sex chromosomes
...
Faceting the plots by TE type to better see which ones are showing a strong pattern:
...
## fig3: Accumulation of TEs

TODO:
- Make data structure that keeps the TE type consistent to a particular colour
- Make the letters for the figures real big
- Clean up the theme for the plots
- Hold off on specifying fig4c - include Bianca's silencing data & look at whether silenced genes have TEs nearby
...
fig3 <- 
  landscapeFraction / (TEsexChromosomes | regionFrequency) / TEfamCountOneToOne +
  plot_layout(heights = c(1.5, 1, 1)) +
  #plot_layout(design = fig3layout) +
...
fig3 #quick preview to double-check

# Save the plot as figure3.png "with sensible defaults"
ggsave("figure3.png", plot = fig3, width = 8, height = 10)
#ggsave("figure3.4.png", plot = fig3, scale = 4)
```
...
test <- H1TEs %>%
  select(scaff, seqOnt, family) %>%
  mutate(scaff = factor(scaff)) %>%
  group_by(scaff) %>%
  summarize(count = n(), .groups = "keep")

glimpse(test)

X <- chisq.test(test$scaff, test$count)
X
X$expected
X$observed
```
...
## TE families in spikes

```{r}
h2biggies <- H2TEs %>%
  filter(scaff == "Y1" | scaff == "Y2") %>%
  filter(family %in% bigFams) %>%
  filter(start > 100000000, start < 300000000)

write_tsv(h2biggies, "bigTEfamilies.H2.100-300Mb")
```
...
ggplot(H1Genes, aes(winnum, n)) +
  geom_line(data = H1Genes, colour = "forestgreen") +
  geom_line(data = H1Lifted, colour = pubColours[2]) +
  geom_line(data = H1GenesLiftedWell, colour = "black") +
  geom_line(data = H2Genes, colour = "navy") +
  geom_line(data = H2Lifted, colour = pubColours[1]) +
  geom_line(data = H2GenesLiftedWell, colour = "grey40") +
  pubTheme +
  labs(x = "H1 Position (1Mb windows)", y = "Number of genes") +
  facet_grid(vars(scaff))
```

```{r testingLiftoverFilter}
ggplot(H1GenesLiftedWell, aes(winnum, amt)) +
  geom_line(data = H1Genes, aes(winnum, amt), colour = "forestgreen") +
...
ggplot(H2GenesLiftedWell, aes(winnum, amt)) +
  geom_line(data = H2Genes, aes(winnum, amt), colour = "navy") +
